...
import type { Availability, Professor, UpsertAvailabilityInput, UpsertPersonInput } from "@/types";
...
export default function ProfessorSection() {
  ...
  useEffect(() => {
    ...
    loadAvailability();
    const channel = supabase
      .channel(`availability-prof-${selectedId}`)
      .on(
        "postgres_changes",
    ...
    if (values.id) {
      ...
      if (values.initialAvailability && values.initialAvailability.length > 0) {
        const rows = values.initialAvailability.map((r) => ({
          person_id: newId,
          person_type: "professor" as const,
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold text-slate-900 dark:text-slate-100">Professors</h2>
      ...
      <div className="bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 rounded-xl shadow">
        ...
        <ul className="divide-y divide-slate-200 dark:divide-neutral-800">
          ...
          {list.length === 0 && !loading ? (
            <li className="p-3 text-sm text-slate-600 dark:text-slate-300">No professors yet</li>
          ) : null}
        </ul>
      </div>
